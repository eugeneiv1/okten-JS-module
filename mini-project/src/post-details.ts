...
const getPost = (postId:number) :Promise<object>=> fetch(`https://jsonplaceholder.typicode.com/posts/${postId}`).then(response => response.json());

const getPostComments = (postId:number) => fetch(`https://jsonplaceholder.typicode.com/posts/${postId}/comments`).then(response => response.json());
...
const showPostInfo = async () => {
    ...
    const post:object = await getPost(+postId);
...
const renderPostInfo = (post:object, tag) => {
...
const renderComments = (comments:[object], tag) => {
    const ul = document.createElement('ul');
...
const saveLikedPostInfo = (post: object):void => {
    const likedPosts:[object] = JSON.parse(localStorage.getItem('likedPosts')) || [];
